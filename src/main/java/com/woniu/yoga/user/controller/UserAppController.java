...
import com.woniu.yoga.user.vo.UserCoach;
...
//        subject.login(token);
    ...
    public Result loginByEmailAndPwd(@RequestBody User user, HttpSession session) {
        ...
        System.out.println("email login user=" + exist);
        if (exist == null) {
...
//-----------------------学生信息------------------------
    ...
    public Result updateStudentInfo(@RequestBody User user, HttpSession session) {
        ...
        Integer userId = ((User) session.getAttribute(SysConstant.CURRENT_USER)).getUserId();
        ...
        session.setAttribute(SysConstant.CURRENT_USER, user);
        return ResultUtil.actionSuccess("成功完善信息", user);
    }
    ...
    @ResponseBody
    public Result updateCoachInfo(@RequestBody UserCoach userCoach, HttpSession session) {

        System.out.println("user coach = " + userCoach);
        User userSession = (User) session.getAttribute(SysConstant.CURRENT_USER);
        User user = new User();
        Coach coach = new Coach();
        user.setUserId(userSession.getUserId());
        user.setIdcard(userCoach.getIdcard());
        user.setUserNickname(userCoach.getUserNickname());
        user.setUserEmail(userCoach.getUserEmail());
        user.setUserPhone(userCoach.getUserPhone());
        user.setUserHeadimg(userCoach.getUserHeadimg());
        user.setUserPrivacy(userCoach.getUserPrivacy());
        user.setRealName(userCoach.getRealName());
        user.setSex(userCoach.getSex());
        user.setUserQq(userCoach.getUserQq());
        user.setUserWechat(userCoach.getUserWechat());
        user.setUserLocation(userCoach.getUserLocation());
        Integer userId = ((User) session.getAttribute(SysConstant.CURRENT_USER)).getUserId();
        user.setUserId(userId);
        ...
        coach.setFullClass(userCoach.getFullClass());
        coach.setCoachDetail(userCoach.getCoachDetail());
        session.setAttribute(SysConstant.CURRENT_USER, user);
        session.setAttribute(SysConstant.CURRENT_COACH, coach);
        Map<String, Object> map = new HashMap<>();
        map.put(SysConstant.CURRENT_COACH, coach);
        map.put(SysConstant.CURRENT_USER, user);
        System.out.println("coach upload ="+coach);
        int row1 = coachService.updateCoach(coach);
        if (row1 < 0 || row1 == 0) {
            return ResultUtil.errorOperation("修改失败");
        }
