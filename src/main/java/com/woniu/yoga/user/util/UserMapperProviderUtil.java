...
public class UserMapperProviderUtil {
    public String findAroundCoachs(SearchConditionDTO searchConditionDTO) {
        ...
        SQL sql = new SQL().SELECT(" real_name realName,user.user_id userId,latitude,longitude,user_headimg headImg,good_comment goodComment,bad_comment badComment,common_comment commonComment,dict_item_name coachStyle ").FROM(" coach,user,base_dirt ");
        //约束：用户ID相等，且角色为瑜伽
        sql.WHERE(" coach.userId = user.userId  and role_id =2");
        //约束：姓名，模糊查询
